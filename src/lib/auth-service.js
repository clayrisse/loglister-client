...
class AuthService {
  ...
  signup( username, password ) {
    const pr = this.auth.post("/auth/signup", { username, password }).then((response) => response.data);
      // .then(({ data }) => data); // Shorter way of `.then((response) => response.data);`
    return pr;
  }

  login( username, password ) {
    const pr = this.auth.post("/auth/login", { username, password }).then((response) => response.data);  
    return pr;
  }
